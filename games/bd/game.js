...
const UNITS = {
  ...
  goblin: {
    team: 'enemy',
    hpMax: 34, dmg: 8, speed: 30, atkRange: 18, atkCdMs: 800,
    aggroRange: 220, radius: 9, color: '#dc2626', edge: '#5a0e0e', icon: '👹',
  },
  goblin_archer: {
    team: 'enemy',
    hpMax: 28, dmg: 9, speed: 25, atkRange: 90, atkCdMs: 1400,
    aggroRange: 230, radius: 8, color: '#ea580c', edge: '#7c2d12', icon: '🏹',
  },
  goblin_mage: {
    team: 'enemy',
    hpMax: 24, dmg: 10, speed: 22, atkRange: 70, atkCdMs: 1700,
    aggroRange: 220, radius: 8, color: '#c084fc', edge: '#581c87', icon: '✨',
    aoeRadius: 32,
  },
  // Катапульта — медленная и хрупкая по AC, но дальнобойная и со сплешем.
  // Главная угроза: если допустить к базе — будет долбить здание/базу с приличным уроном.
  goblin_catapult: {
    team: 'enemy',
    hpMax: 60, dmg: 35, speed: 12, atkRange: 220, atkCdMs: 4200,
  ...
  goblin_boss: {
    ...
    hpMax: 900, dmg: 24, speed: 18, atkRange: 22, atkCdMs: 1100,
...
function startBattle() {
  ...
  state.drag = null;
  state.dragHover = null;
  // Кол-во и темп: w1=8, w5=20, w10=35; интервал 1500→900мс плавно.
  state.enemiesToSpawn = ENEMIES_PER_WAVE + (state.wave - 1) * 3;
...
// Эскалация: к 10-й волне враги +90% HP/DMG. Применяем как bonus в spawnUnit.
function waveEnemyMul(w) { return 1 + (w - 1) * 0.10; }
...
function updateEnemies(dt) {
  ...
  for (const u of state.enemies) {
    ...
    // Катапульта: если ближайший союзник далеко (или его нет), атакует базу с дистанции AoE.
    if (def.canSiege) {
      const { target: aTarget, dist: aDist } = findNearest(u, state.allies);
      const dB = distanceToBaseFront(u);
      const allyClose = aTarget && aDist <= u.atkRange + aTarget.radius;
      if (!allyClose && dB <= u.atkRange) {
        if (u.atkAccum >= u.atkCdMs) {
          u.atkAccum = 0;
          // Удар по базе + AoE по союзникам в радиусе вокруг точки удара.
