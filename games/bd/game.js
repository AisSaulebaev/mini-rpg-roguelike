...
const UNITS = {
  ...
  cave_matriarch: {
    ...
    webAttack: {
      cdMs: 4500, range: 180,
      slowFactor: 0.65, slowDurationMs: 1800,
...
function startBattle() {
  ...
  state.drag = null;
  state.dragHover = null;
  // Кол-во и темп: w1=6, w5=14, w10=24; интервал 1700→1100мс плавно (раньше 1500→900).
  state.enemiesToSpawn = ENEMIES_PER_WAVE + (state.wave - 1) * 2;
  state.enemySpawnEveryMs = Math.max(1100, 1700 - (state.wave - 1) * 70);
...
// Эскалация: к 10-й волне враги +63% HP/DMG. Применяем как bonus в spawnUnit.
function waveEnemyMul(w) { return 1 + (w - 1) * 0.07; }
